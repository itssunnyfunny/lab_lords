...
## 2025-05-14 - [Batch Creation and Return for Seat Allocations]
**Learning:** Found an N+1 query problem where `services/seatAllocation.service.ts` loops through all requested shifts and executes `tx.seatAllocation.create(...)` one-by-one inside a loop when updating allocations. Also, discovered that replacing it with `tx.seatAllocation.createManyAndReturn(...)` correctly batches the inserts and returns the expected result while resolving the issue.
**Action:** Replaced the loop body with array accumulation (`payload.push(...)`) and executed a single `tx.seatAllocation.createManyAndReturn(...)` after the loop. This minimizes the number of database inserts significantly.
