...
## 2024-04-05 - [Eliminate Query Waterfalls in Transactions]
**Learning:** Sequential Prisma `findMany` or `findUnique` queries inside interactive transactions create waterfalls that drastically slow down execution.
**Action:** Use `Promise.all()` to fetch independent entities concurrently before running validation logic, preserving execution speed without compromising transaction integrity.
