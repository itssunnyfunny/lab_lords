...
## 2025-05-24 - [Memory Optimization for Distinct Counting]
**Learning:** Found an O(N) memory and bandwidth overhead in `analytics/student.analytics.ts` where all distinct active student allocations were fetched into application memory via `findMany({ distinct: ["studentId"] })` just to calculate a count using `.length`.
**Action:** Replaced `findMany` with `prisma.student.count` and a relational `some` filter on `seatAllocations` to push the distinct count calculation entirely to the database, resulting in O(1) memory usage in the application and significantly reducing DB bandwidth.
