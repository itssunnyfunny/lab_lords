...
## 2025-05-18 - [Extract FindMany out of Source Allocations Validation Loop]
**Learning:** In `services/shift.service.ts`'s `deleteShift` method (handling `REALLOCATE_BULK`), there was an N+1 query issue inside a validation loop. The code called `tx.seatAllocation.findMany` for every single student in `sourceAllocations` to verify they were not already allocated to the target shift.
**Action:** Lift the query outside the loop. Use `tx.seatAllocation.findMany({ where: { studentId: { in: studentIds } } })` to fetch all relevant active allocations at once, then filter the resulting array in-memory inside the validation loop. This completely eliminates the N+1 query pattern during shift conflict resolution.
