...
## 2024-05-25 - [Batch Permission Upserts Optimization]
**Learning:** Found an O(N) database query bottleneck in `services/staff.service.ts` where permission updates were processed individually inside a loop using `.deleteMany()` and `.upsert()` calls. This resulted in multiple round trips to the database.
**Action:** Replaced the loop with batched array processing, utilizing a single `deleteMany` for the affected actions and a single `createMany` for new permissions, turning O(N) operations into O(1).
