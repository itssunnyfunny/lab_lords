...
## 2025-06-01 - [Memory Optimization for Distinct Related Records]
**Learning:** Found an O(N) memory overhead in `analytics/student.analytics.ts` where we fetched distinct `studentId`s into application memory via `findMany({ distinct: ["studentId"] })` just to calculate the count (`.length`).
**Action:** Replaced `findMany` + `.length` with `.count()` on the parent model (`prisma.student.count`) using a relation filter (`seatAllocations: { some: ... }`). This performs the distinct count directly at the database level, achieving O(1) memory usage.
