...
## 2024-05-18 - Replacing Sequential INSERTs with Prisma `createManyAndReturn`
**Learning:** In scenarios where multiple records need to be created sequentially, using `await tx.model.create` inside a `for...of` loop or a `Promise.all` `.map` results in N+1 database queries, causing significant latency.
**Action:** Accumulate payload data inside loops and use a single `await tx.model.createManyAndReturn` (available in Prisma ^7.8.0) after the loop to batch insertions into one query. If the loop logic requires intra-loop state (like checking for conflicts against previously inserted items), push temporary 'mock' objects to validation arrays to maintain logic integrity while delaying the database hit.
