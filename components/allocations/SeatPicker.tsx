...
interface SeatPickerProps {
    branchId: string;
    studentId?: string; // Optional. If provided, checks if already allocated.
    selectedShift: ShiftCapacity | null;
    selectedSeatId: string | null;
    onSelectShift: (shift: ShiftCapacity) => void;
...
export function SeatPicker({
    ...
    // 2. Fetch seat grid when shift changes
    useEffect(() => {
        if (!selectedShift) {
            setSeatMap(null);
            return;
        ...
        fetch(`/api/branches/${branchId}/shifts/${selectedShift.shiftId}/seat-map`)
            .then(r => {
    ...
    return (
        <div className="space-y-6">
                        ...
                        {shifts.map(shift => {
                            ...
                            return (
                                <div
                                    key={shift.shiftId}
                                    className={cn(
                                        "relative rounded-xl border p-3.5 transition-all text-left w-full",
                                        blocked
                                            ? "border-white/5 bg-white/[0.02] opacity-60 cursor-not-allowed"
                                            : isSelected
                                                ? "border-indigo-500/50 bg-indigo-500/10 shadow-[0_0_15px_rgba(99,102,241,0.15)] cursor-pointer"
                                                : "border-white/10 bg-white/[0.03] hover:border-indigo-500/40 hover:bg-white/[0.06] cursor-pointer"
                                    )}
                                    // Make the entire card focusable and interactive for keyboard a11y
                                    tabIndex={blocked ? -1 : 0}
                                    role="button"
                                    onClick={() => !blocked && onSelectShift(shift)}
                                >
            ...
            {/* Step 2: Seats (only if shift selected) */}
            {selectedShift && (
