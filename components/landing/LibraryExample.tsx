...
export function LibraryExample() {
  ...
  return <div className="example-dashboard example-interactive">
    <div className="example-topbar"><span><BookOpen size={19} aria-hidden="true" /> {t("My library")}</span><span className="example-label">{t("Sample data")}</span></div>
    <div role="tablist" aria-label={t("Example library views")} className="example-tabs">
    ...
    {tabs.map((label, index) => <div key={label} role="tabpanel" id={`${id}-panel-${index}`} aria-labelledby={`${id}-tab-${index}`} tabIndex={0} hidden={selected !== index} className="example-tabpanel">
      {index === 0 ? <><h3>{t("Student records, together.")}</h3><p>{t("Check a student's seat, shift and fee status.")}</p><div className="example-student-list">{students.map(student => <div className="example-student" key={student.name}><span className="example-avatar">{student.initials}</span><div><strong>{student.name}</strong><span>{t("Seat {seat} · {shift}", { seat: student.seat, shift: t(student.shift) })}</span></div><span className={`example-status ${student.state === "Pending" ? "pending" : ""}`}>{t(student.state)}</span></div>)}</div></> : index === 1 ? <><h3>{t("A place for every shift.")}</h3><p>{t("Check availability for the morning shift, 7 am–1 pm.")}</p><SeatExample /></> : <><h3>{t("Know what's paid. See what's pending.")}</h3><p>{t("Recorded fees stay linked to the right student.")}</p><div className="example-student-list">{students.map(student => <div className="example-student" key={student.name}><div><strong>{student.name}</strong><span>{t(student.state === "Paid" ? "Payment recorded" : "Remaining balance")}</span></div><strong className="example-amount">{student.fee}</strong><span className={`example-status ${student.state === "Pending" ? "pending" : ""}`}>{t(student.state)}</span></div>)}</div><p className="example-footnote">{t("Example amounts only. Recording a fee does not charge a student.")}</p></>}
    </div>)}
  </div>;
}
