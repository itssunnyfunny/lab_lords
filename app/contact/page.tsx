...
export default function ContactPage() {
  return <MarketingShell><div className="public-reference-page public-contact">
    <section className="marketing-page-hero"><div className="marketing-container">
      ...
      <div className="marketing-actions mt-7"><a href={`mailto:${siteConfig.supportEmail}`} className="marketing-button"><Mail size={17} aria-hidden="true" />Open email</a><Link href="/support" className="marketing-button-secondary">Get support</Link></div>
      <p className="marketing-lead mt-5">This opens a draft in your email app. Review it and send it yourself. If no email app opens, copy <a className="underline underline-offset-4 break-all" href={`mailto:${siteConfig.supportEmail}`}>{siteConfig.supportEmail}</a> into your preferred email service.</p>
    </div></section>
    <section className="marketing-section"><div className="marketing-container">
      <h2 className="marketing-title">What would you like to know?</h2>
      <div className="marketing-grid mt-8">{questions.map(item => <article key={item.href} className="marketing-card p-7"><h3>{item.title}</h3><p className="marketing-lead mt-4">{item.text}</p><Link className="marketing-text-link mt-5" href={item.href}>{item.link}</Link></article>)}</div>
      <p className="marketing-lead mt-7">You may find your answer in the <Link href="/faq" className="underline underline-offset-4">FAQs</Link>. For an existing account or billing issue, <Link href="/support" className="underline underline-offset-4">visit Support</Link>.</p>
    </div></section>
    <section className="marketing-section public-contact-details"><div className="marketing-container public-two-column grid gap-6 md:grid-cols-2">
      <article className="marketing-card p-6 sm:p-8"><Mail size={24} className="text-[color:var(--ui-form-accent)]" aria-hidden="true" /><h2 className="mt-5 text-xl font-semibold">Contact {siteConfig.name}</h2><a href={`mailto:${siteConfig.supportEmail}`} className="mt-4 inline-block break-all underline underline-offset-4">{siteConfig.supportEmail}</a><p className="mt-4 text-sm leading-7 text-[color:var(--text-secondary)]">For account, billing, refund, privacy or product questions.</p></article>
      <article className="marketing-card p-6 sm:p-8"><MapPin size={24} className="text-[color:var(--ui-form-accent)]" aria-hidden="true" /><h2 className="mt-5 text-xl font-semibold">Business and operational address</h2><p className="marketing-lead mt-4">{siteConfig.businessAddress}</p></article>
    </div></section>
  </div></MarketingShell>;
}
