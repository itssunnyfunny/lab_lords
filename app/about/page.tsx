...
export const metadata = publicMetadata("/about", "About", "Lab Lords helps library, study-hall, coaching and tuition owners keep student records, seats, shifts and fees together.");
const audiences = [
  ["Reading libraries", "Keep the details of a study space together: who uses it, which shift they use and which fees are recorded.", "/software/library-management"],
  ...
  ["Coaching centres", "Keep student and fee administration with the right branch, and give staff access to the work they handle.", "/software/coaching-management"],
  ["Tuition centres", "Start with student records and fee tracking, with separate branch records as your centre grows.", "/software/tuition-management"],
];

export default function AboutPage() {
  return <MarketingShell><div className="public-reference-page public-about">
    <PublicIntro eyebrow="About Lab Lords" title="For the people who keep learning spaces running." description="Lab Lords brings student records, seats, shifts and fees into one workspace for libraries, study halls, coaching centres and tuition centres in India." />
    ...
    <section className="marketing-section public-questions"><div className="marketing-container"><h2 className="marketing-title">Where Lab Lords fits</h2><div className="grid gap-6 md:grid-cols-2 mt-8">{audiences.map(([title, description, href]) => <article key={href} className="marketing-card p-7"><h3>{title}</h3><p className="marketing-lead mt-3">{description}</p><Link href={href} className="marketing-text-link mt-4">Explore {title.toLowerCase()}</Link></article>)}</div></div></section>
