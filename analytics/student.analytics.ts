...
export async function getStudentSeatingSnapshot(
  ...
  // ⚡ Bolt: Replaced findMany({ distinct }) + .length with database-level parent model count
  // Impact: Reduces memory overhead from O(N) to O(1) and eliminates payload transfer for large datasets.
