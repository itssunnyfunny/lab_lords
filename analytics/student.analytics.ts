...
export async function getStudentSeatingSnapshot(
  ...
  // ⚡ Bolt: Memory optimization for distinct related record counting.
  // Impact: Reduces memory overhead from O(N) to O(1) by replacing findMany + distinct + length
  // with a database-level count on the parent model using a relation filter.
