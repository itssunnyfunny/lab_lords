...
export async function getStudentSeatingSnapshot(
  ...
  // ⚡ Bolt: Replaced memory-heavy findMany({ distinct }) + length with database-level relation count
  // Impact: Reduces application memory overhead from O(N) to O(1) and eliminates payload transfer.
