    ...
    static async deleteShift(userId: string, shiftId: string, resolution: ResolutionPlan) {
        ...
        if (resolution.type === "REALLOCATE_BULK") {
            ...
            await prisma.$transaction(async (tx) => {
                ...
                // Impact: Extracts `await tx.seatAllocation.findMany` from the loop, reducing O(n) DB queries to 1.
                ...
                for (const oldAlloc of sourceAllocations) {
                    ...
                    const studentActiveAllocs = allActiveStudentAllocs.filter(sa => sa.studentId === oldAlloc.studentId);
