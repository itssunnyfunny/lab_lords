    ...
    static async assignSeatToShifts(
        ...
        return prisma.$transaction(async (tx) => {
            ...
            for (const requestedShift of requestedShifts) {
                ...
                // 8. Queue allocation for this shift (with optional multiShiftId)
                const payload = {
                    seatId,
                    studentId,
                ...
                // also see allocations queued earlier in this transaction.
                ...
                allocationsToCreate.push(payload);
            }

            // ⚡ Bolt: Optimizing bulk allocation creation
            // Impact: Replaced N sequential inserts with a single createManyAndReturn bulk operation
            const createdAllocations = allocationsToCreate.length > 0
                ? await tx.seatAllocation.createManyAndReturn({ data: allocationsToCreate })
                : [];
